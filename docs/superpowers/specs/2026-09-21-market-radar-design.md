...
> ⚠️ **살아있는 문서(Living document) — 언제든 변경될 수 있다.**
> 마켓 레이더는 이 시스템의 **핵심 로직**(발굴·점수·소싱 판정)을 담당하므로, 실데이터·검증 결과·사용자 판단에 따라
> 점수 공식·가중치·수집 규모·데이터 모델·소싱 조건이 **수시로 개정된다.** 개정 시 이 문서를 먼저 고치고(사유·날짜 남김),
> 관련 회귀 테스트를 함께 갱신한다. 이 문서가 "지금 합의된 기준"이며, 확정 불변이 아니다.
>
> 변경 이력: 2026-09-21 최초 작성.
